...
function buildPallet(widthMM,lengthMM,numbered=false){
 ...
 for(let i=0;i<deckCount;i++){const z=-L/2+deckW/2+i*(L-deckW)/(deckCount-1);board(g,W,deckH,deckW,0,topY,z);if(numbered){const n=label(String(i+1),.46);n.position.set(-W*.27,topY+.35,z);g.add(n)}}
